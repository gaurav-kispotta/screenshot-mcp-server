...
export function setupRoutes(
  ...
  router.get('/windows/active', async (req: Request, res: Response) => {
    try {
      ...
      if (window) {
        ...
        res.status(404).json({ success: false, error: 'No active window found' });
      }
    } catch (error) {
      auditLogger.log(
        req.ip || 'unknown',
      ...
      res.status(500).json({ success: false, error: 'Failed to get active window' });
    }
  });
  ...
  router.get('/applications/:appName/windows', async (req: Request, res: Response) => {
    try {
      const windows = await windowManager.getWindowsByApplication(req.params.appName);
      ...
      res.status(500).json({ success: false, error: 'Failed to get application windows' });
    }
  });
  ...
  router.get('/screenshots/screen', async (req: Request, res: Response) => {
    try {
      ...
      const processedBuffer = await processImageIfRequested(buffer, req.query, imageProcessor);
  ...
  router.get('/screenshots/window/:id', async (req: Request, res: Response) => {
    try {
      const options = parseScreenshotOptions(req.query);
      const buffer = await screenshotEngine.captureWindow(req.params.id, options);
      ...
      res.status(500).json({ success: false, error: 'Failed to capture window screenshot' });
    }
  });
  ...
  router.get('/screenshots/region', async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ success: false, error: 'Failed to capture region screenshot' });
    }
  });
